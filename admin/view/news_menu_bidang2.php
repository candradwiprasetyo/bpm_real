...
<table width="100%" border="0" cellspacing="0" cellpadding="2" class="table">
	...
	$key="";
		  ...
		  while($r_per2 = mysql_fetch_object($q_per2)){
			  if($no == 1){
				  $cat2 .= " and id_menu = ".$r_per2->id_menu;
			  }else{
			  $cat2 .= " or id_menu = ".$r_per2->id_menu;
			  }
			  $no++;
		  }
	//echo $cat2;
...
$query = "select count(*) from news_menu a 
					...
					where b.level = '3' $cat2 $where order by news_date desc";
...
$query = "select * from news_menu a 
					join menus b on b.id_menu = a.news_cat_id
					where b.level = '3' $cat2 $where order by news_date desc $limit";
$result = mysql_query($query);
$i = 1;
while($b=mysql_fetch_array($result)){
    ...
    <tr <?php if($i%2 == 1){ ?>class="tr" <?php }else{ ?> class="tr2"<?php } ?>>
     ...
     <a href="admin.php?page=admin/view/news_menu_bidang2&news_id=<?php echo $b[0] ?>"><div class="edit"></div></a>
