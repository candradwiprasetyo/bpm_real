...
<form name="form1" method="post" enctype="multipart/form-data" action="<?php if($_GET['publication_id']){ echo "admin/controller/east_java_investment.php?req=edit&publication_id=$_GET[publication_id]"; }else{ echo "admin/controller/east_java_investment.php?req=save"; } ?>" class="form">
...
$query = "SELECT count(*) FROM publications where publication_type = '1' $where order by publication_title ";
...
$query = "SELECT * FROM publications where publication_type = '1' $where order by publication_date DESC $limit";
$result = mysql_query($query);
$i = 1;
while($b=mysql_fetch_array($result)){
    ...
    <tr <?php if($i%2==1){ echo "class='tr'"; }else{ echo "class='tr2'"; } ?> >
      ...
      <a href="javascript:void(0)" onclick="confirm_delete(<?php echo $b[0]; ?>,'admin/controller/east_java_investment.php?req=delete&publication_id=')"><div class="trash"></div></a>
     <a href="admin.php?page=admin/view/east_java_investment&publication_id=<?php echo $b[0] ?>"><div class="edit"></div></a>
      <a href="admin.php?page=admin/view/east_java_investment_pic&publication_id=<?php echo $b[0] ?>" title="Tambah Foto"><div class="add"></div></a>
      
    <?php 
	if($_SESSION['user_type_type'] == 0 || $_SESSION['user_type_type'] == 3){
	if($b['active_status'] == 0){?>
      <a href="admin/controller/east_java_investment.php?req=act&publication_id=<?php echo $b[0] ?>" title="Tampilkan"><div class="act"></div></a>
      <?php
	 }else{
	  ?>
       <a href="admin/controller/east_java_investment.php?req=deact&publication_id=<?php echo $b[0] ?>" title="Tidak ditampilkan"><div class="deact"></div></a>
      <?php
	 }
}
	  ?>
...
if ($pageno2 == 1) {
   ...
   echo "<a href='{$_SERVER['PHP_SELF']}?page=admin/view/east_java_investment&pageno2=1'><div class=\"page_first\"></div></a> ";
   $prevpage = $pageno2-1;
   echo "<a href='{$_SERVER['PHP_SELF']}?page=admin/view/east_java_investment&pageno2=$prevpage'><div class=\"page_prev\"></div> </a>";
} // if
...
if ($pageno2 == $lastpage) {
   ...
   $nextpage = $pageno2+1;
   echo "<a href='{$_SERVER['PHP_SELF']}?page=admin/view/east_java_investment&pageno2=$nextpage'><div class=\"page_next\"></div></a> ";
   echo "<a href='{$_SERVER['PHP_SELF']}?page=admin/view/east_java_investment&pageno2=$lastpage'><div class=\"page_last\"></div></a> ";
} 
