...
class PDF extends FPDF{
	function Header(){
		...
		$this->Cell(1,1.2,'Master Supplier');
		$this->Ln();
		$this->SetFont('Arial','B',8);
...
$cri = "";
	...
	while($row=mysql_fetch_array($sql)){
		if($t%2==1){
			...
			$pdf->Cell(4,1,$row[3],0,0,'L',1);
			$pdf->Cell(4,1,$row[4],0,0,'L',1);
			$pdf->Ln();
		}else{
			$pdf->Cell(2,1,$row[0],0,0,'L');
			$pdf->Cell(5,1,$row[1],0,0,'L');
			$pdf->Cell(4,1,$row[2],0,0,'L');
			$pdf->Cell(4,1,$row[3],0,0,'L');
			$pdf->Cell(4,1,$row[4],0,0,'L');
			$pdf->Ln();
		}
		$t++;
	}
