...
class PDF extends FPDF{
	function Header(){
		...
		$this->Cell(1,1.2,'Transaksi Cash');
		$this->Ln();
	}
	function Footer(){
...
$cri = "";
	if($_GET['id']){
		$cri = "where id_cash ='".$_GET['id']."'";
	}
	$sql = mysql_query("select * from cash ".$cri."");
	...
	$bc = mysql_fetch_array($ac);
			...
			$pdf->Cell(2,0.8,'ID Cash',0,0,'L');
			$pdf->Cell(3,0.8,':',0,0,'R');
			$pdf->Cell(2,0.8,$row['id_cash'],0,0,'L');
			$pdf->Ln();
			$pdf->SetFillColor(224,255,255);
			...
			$pdf->Ln();
			$pdf->SetFillColor(224,255,255);
			$pdf->Cell(2,0.8,'Harga',0,0,'L');
			$pdf->Cell(3,0.8,':',0,0,'R');
			$pdf->Cell(2,0.8,number_format($row['harga'],2),0,0,'L');
			$pdf->Ln();
			$pdf->SetFillColor(224,255,255);
			$pdf->Cell(2,0.8,'Bayar',0,0,'L');
			$pdf->Cell(3,0.8,':',0,0,'R');
			$pdf->Cell(2,0.8,number_format($row['bayar'],2),0,0,'L');
			$pdf->Ln();
			$pdf->SetFillColor(224,255,255);
			$pdf->Cell(2,0.8,'Kembali',0,0,'L');
			$pdf->Cell(3,0.8,':',0,0,'R');
			$pdf->Cell(2,0.8,number_format($row['kembali'],2),0,0,'L');
			$pdf->Ln();
			$pdf->SetFillColor(224,255,255);
