...
class PDF extends FPDF{
	function Header(){
		...
		$this->Cell(1,1.2,'Master Customer');
		$this->Ln();
		$this->SetFont('Arial','B',8);
		...
		$this->Cell(5,1,'Telp','B',0,'');
		$this->Cell(3,1,'Kota','B',0,'');
		$this->Ln();
	}
	function Footer(){
...
$cri = "id_cust<>0";
	if($_GET['nama']){
		$cri = "nama like '%".$_GET['key']."%' and id_cust<>0";
	}
	$sql = mysql_query("select * from customer where ".$cri."");
	$t = 0;
	while($row=mysql_fetch_array($sql)){
		if($t%2==1){
			...
			$pdf->Cell(4,1,$row['alamat'],0,0,'L',1);
			$pdf->Cell(5,1,$row['telp'],0,0,'L',1);
			$pdf->Cell(3,1,$row['kota'],0,0,'L',1);
			$pdf->Ln();
		}else{
			$pdf->Cell(2,1,$row[0],0,0,'L');
			$pdf->Cell(5,1,$row[1],0,0,'L');
			$pdf->Cell(4,1,$row['alamat'],0,0,'L');
			$pdf->Cell(5,1,$row['telp'],0,0,'L');
			$pdf->Cell(3,1,$row['kota'],0,0,'L');
			$pdf->Ln();
		}
		$t++;
	}
