...
$cri = "";
	if($_GET['id']){
		$cri = "where id_kolektor='".$_GET['id']."'";
	}
	$sql=  mysql_query("select * from collector ".$cri."");
	$row = mysql_fetch_array($sql);
			$pdf->SetFillColor(224,255,255);
			$pdf->Cell(1,0.8,'ID Kolektor',0,0,'L');
			$pdf->Cell(3,0.8,':',0,0,'R');
			$pdf->Cell(6,0.8,$row['id_kolektor'],0,0,'L');
			$pdf->Ln();
			$pdf->SetFillColor(224,255,255);
			...
			$pdf->Cell(3,0.8,':',0,0,'R');
			$pdf->Cell(6,0.8,$row['tgl_masuk'],0,0,'L');
			$pdf->Ln();
