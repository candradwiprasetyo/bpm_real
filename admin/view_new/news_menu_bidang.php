...
<form name="RTEDemo" method="post" enctype="multipart/form-data" action="<?php if($_GET['news_id'] !=""){ echo "admin/controller/news_menu_bidang.php?req=edit&news_id=".$_GET['news_id']; }else{ echo "admin/controller/news_menu_bidang.php?req=save"; } ?>" class="form" onsubmit="return submitForm();">
 <table width="800" border="0" cellspacing="0" cellpadding="2">
    <tr>
    	<td>KATEGORI BIDANG</td>
    	<td><select name="i_smenu" id="i_smenu" class="list">
    	  <?php
      $quti = mysql_query("select * from menus where level = '3' order by id_menu");
...
$query = "select count(*) from news_menu a 
					...
					where b.level = '3' order by news_id desc  ";
...
$query = "select * from news_menu a 
					join menus b on b.id_menu = a.news_cat_id
					where b.level = '3' order by news_id desc $limit";
$result = mysql_query($query);
$i = 1;
while($b=mysql_fetch_array($result)){
		
		$q_bidang = mysql_query("select name from menus where id_menu = '".$b['news_cat_id']."'");
		$r_bidang = mysql_fetch_array($q_bidang);
	?>
    
    <tr <?php if($i%2 == 1){ ?>class="tr" <?php }else{ ?> class="tr2"<?php } ?>>
      <td height="28" class="td"><?php echo $i ?></td>
        <td  class="td"><?php echo $r_bidang['name'] ?></td>
      <td  class="td"><?php echo $b[1] ?></td>
      <td class="td"><?php echo $b[3] ?></td>
      <td class="td"><img src="<?php echo $b['news_img'] ?>" height="20" /></td>
      <td  class="td">
      <a href="javascript:void(0)" onclick="confirm_delete(<?php echo $b[0]; ?>,'admin/controller/news_menu_bidang.php?req=delete&news_id=')"><div class="trash"></div></a>
     <a href="admin.php?page=admin/view_new/news_menu_bidang&news_id=<?php echo $b[0] ?>"><div class="edit"></div></a>
        <?php if($b['active_status'] == 0){?>
      <a href="admin/controller/news_menu_bidang.php?req=act&news_id=<?php echo $b[0] ?>" title="Tampilkan"><div class="act"></div></a>
      <?php
	 }else{
	  ?>
       <a href="admin/controller/news_menu_bidang.php?req=deact&news_id=<?php echo $b[0] ?>" title="Tidak ditampilkan"><div class="deact"></div></a>
      <?php
	 }
	  ?>
      </td>
...
if ($pageno2 == 1) {
   ...
   echo "<a href='{$_SERVER['PHP_SELF']}?page=admin/view_new/news_menu_bidang&pageno2=1'><div class=\"page\"> FIRST</div></a> ";
   $prevpage = $pageno2-1;
   echo "<a href='{$_SERVER['PHP_SELF']}?page=admin/view_new/news_menu_bidang&pageno2=$prevpage'><div class=\"page\"> PREV</div> </a>";
} // if
...
if ($pageno2 == $lastpage) {
   ...
   $nextpage = $pageno2+1;
   echo "<a href='{$_SERVER['PHP_SELF']}?page=admin/view_new/news_menu_bidang&pageno2=$nextpage'><div class=\"page\"> NEXT</div></a> ";
   echo "<a href='{$_SERVER['PHP_SELF']}?page=admin/view_new/news_menu_bidang&pageno2=$lastpage'><div class=\"page\"> LAST</div></a> ";
} 
