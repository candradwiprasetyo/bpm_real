...
<div class="container floated">
	...
	<div class="sixteen floated page-title">
		...
		<h2>Potensi dan peluang Investasi Jawa Timur</h2>
...
<div class="container floated">
				...
				<div class="color-1">
					<ul>
						...
						if(isset($_POST['search_publication'])){
						 ...
						 $query = "SELECT * FROM publications where publication_type = '4' and active_status = '1' $where ORDER BY publication_title";
						 $excute = mysql_query($query);
						  while($data = mysql_fetch_array($excute)){
						?>
						<a href="index.php?page=buku_biru&publication_id=<?= $data['publication_id']?>&title=<?= $data['publication_title']?>">        
	...
	if(abs((int)$_GET['publication_id']) == ""){
		$q_max = mysql_query("select max(publication_id) as new_id from publications where publication_type = '4' and active_status = '1'");
